...
impl VmfFile {
    ...
    pub fn get_entities_in_visgroup<'a>(
        ...
        let iterator = self.entities.iter()
            .chain(self.hiddens.iter())
            .filter(move |entity| {
                // Check if the entity belongs to one of the target VisGroup IDs.
                entity.editor.visgroup_id.map_or(false, |ent_group_id| {
                    ids_to_check.contains(&ent_group_id)
                })
            });
    ...
    pub fn get_entities_in_visgroup_mut<'a>(
        ...
        group_id: i32,
        include_children: bool,
    ) -> Option<impl Iterator<Item = &'a mut Entity> + 'a> { // Note: returns mutable references
        ...
        let iterator = self.entities.iter_mut() // Get mutable iterator
    ...
    pub fn get_solids_in_visgroup<'a>(
        ...
        let iterator = self.world.solids.iter()
    ...
    pub fn get_solids_in_visgroup_mut<'a>(
        ...
        let iterator = self.world.solids.iter_mut() // Mutable iterator
